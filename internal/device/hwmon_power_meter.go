...
// HwmonOptionFn is a function that configures hwmonPowerMeter
...
// WithHwmonReader sets the hwmonReader used by hwmonPowerMeter
...
// WithHwmonZoneFilter sets zone names to include for monitoring
...
func (h *hwmonPowerMeter) Init() error {
	...
	// try reading power from the first zone and return the error
	_, err = zones[0].Power()
	return err
}

func (h *hwmonPowerMeter) needsChipFiltering() bool {
	return len(h.chipFilter) != 0
}
...
// filterZones applies chip and zone filters
func (h *hwmonPowerMeter) filterZones(zones []EnergyZone) []EnergyZone {
	if !h.needsChipFiltering() && !h.needsZoneFiltering() {
		return zones
	}

	chipWanted := make(map[string]bool)
	for _, name := range h.chipFilter {
		chipWanted[strings.ToLower(name)] = true
	}
	...
	for _, zone := range zones {
		...
		// Check chip filter
		if h.needsChipFiltering() && !chipWanted[strings.ToLower(hwmonZone.chipName)] {
			excluded = append(excluded, zone.Name())
			continue
		}

		// Check zone filter
		if h.needsZoneFiltering() && !zoneWanted[strings.ToLower(zone.Name())] {
			excluded = append(excluded, zone.Name())
			continue
		}
...
// sysfsHwmonReader implements hwmonReader by reading from sysfs
...
// findSensorsByType finds all sensors of a given type (e.g., "power", "energy")
...
func (z *hwmonPowerZone) Power() (float64, error) {
