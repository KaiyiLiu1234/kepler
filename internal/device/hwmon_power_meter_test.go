...
func TestHwmonPowerMeter_Zones(t *testing.T) {
	...
	for i, zone := range zones {
		...
		assert.NoError(t, err, "Energy() should not return error")
...
func TestHwmonPowerMeter_ZoneDetails(t *testing.T) {
	...
	for _, zone := range zones {
		...
		require.NoError(t, err)
		t.Logf("  Current Power: %.3f W (%.0f µW)", power, power*1000000)

		// Verify power is reasonable (between 0 and 500W for test data)
		assert.GreaterOrEqual(t, power, 0.0, "Power should be non-negative")
		assert.LessOrEqual(t, power, 500.0, "Power should be reasonable for test data")
	}
}
...
func TestHwmonPowerMeter_PowerReadings(t *testing.T) {
	...
	for _, zone := range zones {
		...
		if ok {
			assert.InDelta(t, expected, power, 0.01,
				"Zone %s: expected %.2f W, got %.2f W", zone.Name(), expected, power)
			t.Logf("✓ Zone '%s': %.2f W (matches expected)", zone.Name(), power)
		} else {
			t.Logf("  Zone '%s': %.2f W", zone.Name(), power)
		}
	}
}
...
// TestHwmonPowerMeter_ChipFilter tests chip filtering
func TestHwmonPowerMeter_ChipFilter(t *testing.T) {
	t.Logf("\n=== Testing Chip Filtering ===")

	// Test with k10temp chip filter
	meter, err := NewHwmonPowerMeter(validHwmonPath, WithChipFilter([]string{"k10temp"}))
	require.NoError(t, err)

	zones, err := meter.Zones()
	require.NoError(t, err)

	t.Logf("With chip filter 'k10temp', found %d zones:", len(zones))
	for _, zone := range zones {
		hwmonZone := zone.(*hwmonPowerZone)
		t.Logf("  - %s (chip: %s)", zone.Name(), hwmonZone.chipName)
		assert.Equal(t, "k10temp", hwmonZone.chipName,
			"All zones should be from k10temp chip")
	}

	// Test with amdgpu chip filter
	meter2, err := NewHwmonPowerMeter(validHwmonPath, WithChipFilter([]string{"amdgpu"}))
	require.NoError(t, err)

	zones2, err := meter2.Zones()
	require.NoError(t, err)

	t.Logf("\nWith chip filter 'amdgpu', found %d zones:", len(zones2))
	for _, zone := range zones2 {
		hwmonZone := zone.(*hwmonPowerZone)
		t.Logf("  - %s (chip: %s)", zone.Name(), hwmonZone.chipName)
		assert.Equal(t, "amdgpu", hwmonZone.chipName,
			"All zones should be from amdgpu chip")
	}
}
...
func TestHwmonPowerMeter_MultipleReadings(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		require.NoError(t, err)
		t.Logf("  Reading %d: %.2f W", i+1, power)
	}
}
...
func TestHwmonPowerMeter_FallbackNaming(t *testing.T) {
	...
	for _, zone := range zones {
		...
		if hwmonZone.chipName == "platform_sensor" {
			...
			require.NoError(t, err)
			t.Logf("  Power: %.2f W", power)
			assert.InDelta(t, 30.0, power, 0.01, "Power should be 30W")
		}
	}
...
func TestHwmonPowerMeter_PreferAverage(t *testing.T) {
	...
	for _, zone := range zones {
		if zone.Name() == "gpu" {
			...
			t.Logf("  Actual reading: %.2f W", power)

			assert.InDelta(t, 118.0, power, 0.01,
...
func TestHwmonPowerZone_Interface(t *testing.T) {
	...
	power, err := zone.Power()
	assert.NoError(t, err)
	assert.InDelta(t, 45.0, power, 0.01, "Power should be 45W from test file")
